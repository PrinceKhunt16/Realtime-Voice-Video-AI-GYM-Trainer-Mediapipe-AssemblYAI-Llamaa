...
from utils.var import POSE_CONNECTIONS
from exercise_class.squat import SquatDetector
from exercise_class.pushup import PushUpDetector
from exercise_class.bicep_curl import BicepCurlDetector
from exercise_class.shoulder_press import ShoulderPressDetector
from exercise_class.lunges import LungeDetector


# Manual Skeleton Connections
# (From MediaPipe PoseLandmark indices)
...
class ExerciseVideoProcessor(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        # Detect landmarks
        self._frame_timestamp_ms += int(1000 / 30) # Approx 30fps increment
        result = self._landmarker.detect_for_video(mp_image, self._frame_timestamp_ms)

        if result.pose_landmarks:
            landmarks = result.pose_landmarks[0] # Take first person detected
            ...
            if detector:
                try:
                    # Pass the landmarks list directly to the detector
